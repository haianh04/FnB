...
export default function MoreMenu({ onNavigate, onLogout, user }) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 pt-12 px-3 pb-2 font-sans relative">
        ...
        {menuItems.map((item) => (
          <button key={item.id} onClick={() => onNavigate(item.id)} className="w-full bg-white p-3 rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.02)] border border-gray-100 flex items-center justify-between active:scale-[0.98] transition-all hover:border-orange-200 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center border border-gray-100 group-hover:bg-white transition-colors">{item.icon}</div>
              <div className="text-left"><p className="font-bold text-gray-800 text-base">{item.label}</p><p className="text-xs text-gray-400 mt-0">{item.desc}</p></div>
            </div>
